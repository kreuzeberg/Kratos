...
public:
    ...
    std::string Info() const override { return "ApplyConstantInterpolateLinePressureProcess"; }
...
private:
                                   ...
                                   bool                      isBottom = false)
    {
        ...
        if (!LeftBoundaryNodes.empty() && !RightBoundaryNodes.empty()) {
            ...
            KRATOS_ERROR << "There is not enough points around interpolation, node Id" << rNode.Id()
                         << std::endl;
        }
    }
                                           ...
                                           std::vector<Node*>&       rFoundNodes) const
    {
        ...
        for (auto boundary_node : rBoundaryNodes) {
            ...
            if (std::abs(CoordinatesBoundary[mHorizontalDirection] - HorizontalCoordinate) <=
    ...
    Node* FindClosestNodeOnBoundaryNodes(const Node& rNode, const std::vector<Node*>& BoundaryNodes, const bool isBottom)
    {
        ...
        for (auto boundary_node : BoundaryNodes) {
            if (std::abs(boundary_node->Coordinates()[mHorizontalDirection] - HorizontalCoordinate) <=
                horizontalDistance) {
                horizontalDistance =
